...
var preCheck = function(opt, check, type, optional) {
        if(!!opt || optional) {
                if (!!opt && typeof opt !== type) {
                         throw "Wrong " + check + " type";
                }
        } else {
                throw "Wrong " + check;
        }
...
var MethodNotAllowedError = 405;
var NotAuthorizedError = 401;
var ResourceNotFoundError = 404;

///--- Functions

var serveStatic = function(opts) {
        opts = opts || {};
        ...
        preCheck(opts, 'options', 'object', false);
        preCheck(opts.directory, 'options.directory', 'string', false);
        preCheck(opts.maxAge, 'options.maxAge', 'number', true);
        preCheck(opts.match, 'options.match', 'object', true);

        var p = path.normalize(opts.directory).replace(/\\/g, '/');
        var re = new RegExp('^' + p + '/?.*');

        var serveFileFromStats = function(file, err, stats, req, res, next) {
                if (err) {
        ...
        var serve = function(req, res, next) {
                console.log('file:'+opts.subst(req.path()));
                var file = path.join(opts.directory, !!opts.subst ? opts.subst(req.path()) : req.path());
